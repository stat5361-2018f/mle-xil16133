...
$$
where $C$ is a constant. Here we use Nelder-Mead method to solve this problem.
```{r}
y <- beetles1$beetles
t <- beetles1$days
n <- length(y)
...
grr <- function(x){
  ...
  c(n/2/x1-sum((log(y)-log(2*x2/(2+(x2-2)*exp(-x3*t))))^2)/2/x1^2,
    sum((log(y)-log(2*x2/(2+(x2-2)*exp(-x3*t))))*(2*exp(-x3*t)-2)/(2+(x2-2)*exp(-x3*t)))/x2/x1,
    sum((log(y)-log(2*x2/(2+(x2-2)*exp(-x3*t))))*((2-x2)*t*exp(-x3*t))/(2+(x2-2)*exp(-x3*t)))/x1
  )
}

mod1 <- optim(c(1,1900,0.1), fr, grr, method = "BFGS", hessian = TRUE)
mod1 <- optim(c(1,2100,0.1), fr, grr, method = "CG", hessian = TRUE)
mod1 <- constrOptim(c(1,1200,0.1), fr, grr, ui=diag(nrow = 3), ci=c(0,0,0), hessian = TRUE)
